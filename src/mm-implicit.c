...
void *mm_malloc(size_t size) {
    ...
    // coalesce here (seems very inefficient tho)
    block_t *curr_block = mm_heap_first;
    while (mm_heap_last != NULL && curr_block <= mm_heap_last) {
        if (!is_allocated(curr_block)) {
            block_t *next_block = (void *) curr_block + get_size(curr_block);
            size_t new_block_size = get_size(curr_block);
            while (!is_allocated(next_block) && next_block <= mm_heap_last) {
                new_block_size += get_size(next_block);
                next_block = (void *) next_block + get_size(next_block);
            }
            set_header(curr_block, new_block_size, false);
            curr_block = next_block;
        }
        // go to next block in all cases
        curr_block = (void *) curr_block + get_size(curr_block);
    }

    // If there is a large enough free block, use it
    // to implement splitting here
    block_t *block = find_fit(size);
    if (block != NULL) {
        if (get_size(block) > size + sizeof(block_t)) {
            block_t *free_split_block = (void *) block + size;
            size_t free_block_size = get_size(block) - size;
            set_header(block, size, true);
            set_header(free_split_block, free_block_size, false);
            if (block == mm_heap_last) {
                mm_heap_last = free_split_block;
            }
        }
        else {
...
void *mm_realloc(void *old_ptr, size_t size) {
    ...
    memcpy(ptr, old_ptr, min(get_size(old_block), get_size(new_block)) - sizeof(block_t));
    mm_free(old_ptr);
    return ptr;
}
